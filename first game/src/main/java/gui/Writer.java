...
public final class Writer {
  ...
  public static void write(Writable writable, WritingSpecifications specifications) {
    if (Game.getGame().getGameWindow() != null) { // There will be no window when running the tests, so check to prevent a NPE.
      Game.getGame().getGameWindow().scheduleWriteToTextPane(writable, specifications);
